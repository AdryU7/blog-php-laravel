...
class CommentController extends Controller
{
    ...
    {
        //Verify in the article if there is a comment from the user
        $result = Comment::where('user_id', Auth::user()->id)
                         ->where('article_id', $request->article_id)->exists();
        
        //Query to obtain the slug and status from the commented article
        $article = Article::select('status', 'slug')->find($request->article_id);

        //If it does not exist and if article status is public, then they can comment.
